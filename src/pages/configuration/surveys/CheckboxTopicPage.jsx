...
import { checkboxDocsDictionary } from "../../../data/checkboxDocsData";

export default function CheckboxTopicPage({ topicKey }) {
  const key = topicKey || "overview";
  const data = checkboxDocsDictionary[key] || checkboxDocsDictionary["overview"];
  ...
  return (
    <DocPage
      path={data.path}
      eyebrow="Survey Builder › Checkbox Field"
      title={data.title}
    ...
    >
      ...
      <Section id="main-features" title="Main Features">
        <div className="space-y-3 text-[14px] leading-6">
          <div className="p-4 rounded-xl border border-ink-900/10 dark:border-[#262626] bg-white dark:bg-[#0A0A0A]">
            <p className="font-semibold text-ink-900 dark:text-[#FFFFFF]">Configurable Parameters</p>
      ...
      <Section id="related-modules" title="Related Modules">
        ...
        <div className="grid gap-2 sm:grid-cols-3 text-xs font-medium">
          <Link to="/configuration/surveys/field-dropdown" className="p-3 rounded-lg border border-ink-900/10 dark:border-[#262626] hover:bg-cyan-500/5 text-cyan-700 dark:text-cyan-400">
          ...
          <Link to="/configuration/surveys/validation-rules" className="p-3 rounded-lg border border-ink-900/10 dark:border-[#262626] hover:bg-cyan-500/5 text-cyan-700 dark:text-cyan-400">
            Validation Rules →
          </Link>
        </div>
      </Section>
