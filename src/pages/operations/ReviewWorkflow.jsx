...
export default function ReviewWorkflow() {
  return (
    <DocPage
      ...
      description="Operational review process — add inspection comments, update inspection status, track revision history, and print or export reports."
      toc={toc}
      hideVideo={true}
    >
      ...
      <Section id="comments" title="Comments">
        ...
        <div className="my-4 grid gap-3 sm:grid-cols-2">
          <div className="p-4 rounded-lg border border-ink-900/10 dark:border-[#262626] bg-white dark:bg-[#0A0A0A]">
            <p className="font-semibold text-sm text-ink-900 dark:text-[#FFFFFF]">Internal Inspection Notes</p>
      ...
      <Section id="status-updates" title="Status Updates">
        <p>
          Coordinators execute state machine transitions by selecting one of the official review status decisions. Each decision triggers downstream actions in the system.
        </p>
        ...
        <div className="my-4 space-y-3">
          ...
          <div className="p-3.5 rounded-lg border border-amber-200 dark:border-amber-500/30 bg-amber-50/30 dark:bg-amber-500/10">
            ...
            <p className="text-xs text-ink-700 dark:text-[#E5E5E5] mt-1.5">Inspection is conditionally approved with minor remarks, non-critical evidence flags, or location overrides. Data is locked and forwarded for report generation while retaining conditional flags for audit tracking.</p>
          </div>
          <div className="p-3.5 rounded-lg border border-rose-200 dark:border-rose-500/30 bg-rose-50/30 dark:bg-rose-500/10">
            ...
            <p className="text-xs text-ink-700 dark:text-[#E5E5E5] mt-1.5">Survey fails verification (e.g., severe location mismatch, incorrect evidence, or incomplete mandatory fields). A push notification is dispatched to the surveyor requesting corrected re-submission.</p>
          </div>
        </div>
          ...
          <table className="w-full text-left text-xs border border-ink-900/10 dark:border-[#262626]">
            ...
            <tbody className="divide-y divide-ink-900/5 dark:divide-[#262626] text-ink-700 dark:text-[#E5E5E5]">
              ...
              <tr>
                ...
                <td className="p-2.5">New field value(s) that replaced the previous values, highlighted to draw attention.</td>
              </tr>
              <tr>
                <td className="p-2.5 font-mono font-medium">Changed By</td>
                <td className="p-2.5">Full name and user role of the person who made the change — field surveyor (re-submission) or portal coordinator (manual correction).</td>
              </tr>
              <tr>
              ...
              <tr>
                ...
                <td className="p-2.5">Optional commentary explaining the reason for the change or documenting an override justification.</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <Section id="print-report" title="Print Report">
        <p>
          The <strong>Print Report</strong> feature allows coordinators to generate a printable version of the inspection detail directly from the Inspection Detail Page. It is accessible via the action menu, typically after the inspection has been reviewed.
        </p>

        <div className="my-4 space-y-3">
          {[
            { title: "Preview", desc: "Opens a print-layout preview of the inspection record, formatted to standard A4 paper dimensions. Includes the inspection header, all field responses, GPS summary, and inspection notes." },
            ...
            { title: "Close Preview", desc: "Dismisses the print preview overlay and returns the coordinator to the Inspection Detail Page without performing any print or download action." },
          ].map(({ title, desc }) => (
        ...
        <Callout type="note">
          The Print Report generates an internal summary. For the official customer-facing inspection certificate, use the <Link to="/reports/report-builder" className="text-cyan-600 dark:text-cyan-400 underline">Report Builder</Link> which applies the branded report template configured in the Inspection Template.
        </Callout>
      </Section>
    </DocPage>
  );
